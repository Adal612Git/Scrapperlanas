...
RENDER_DIR="/tmp/scrapperlanas-workflows"
...
IMPORT_WEBHOOK_PATH="${N8N_IMPORT_WEBHOOK_PATH:-scrapperlanas/import}"
SCRAPPERLANAS_BASE_URL="${SCRAPPERLANAS_BASE_URL:-http://web:8000}"
...
render_workflows() {
  base_url="$(printf '%s' "${SCRAPPERLANAS_BASE_URL}" | sed 's:/*$::')"
...
if [ "${FORCE_BOOTSTRAP}" = "true" ] || [ ! -f "${BOOTSTRAP_MARKER}" ]; then
  echo "[scrapperlanas] Rendering bundled n8n workflows into ${RENDER_DIR}"
  render_workflows
  echo "[scrapperlanas] Importing bundled n8n workflows from ${RENDER_DIR}"
